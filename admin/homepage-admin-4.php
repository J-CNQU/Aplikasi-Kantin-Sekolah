...
if (isset($_GET['delete_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: homepage-admin-4.php?counter=$selected_counter_num&msg=deleted"); 
        exit();
    } else {
...
<body class="site-bg">
    <nav class="navbar">
        ...
        <ul class="nav-links">
            <li><a class="active" href="homepage-admin-4.php">Kelola Menu Counter</a></li>
    ...
    <div class="container">
        ...
        <form method="POST" action="homepage-admin-4.php?counter=<?php echo $selected_counter_num; ?>">
            <?php if ($edit_item): ?>
                <input type="hidden" name="id" value="<?php echo $edit_item['id']; ?>">
            <?php endif; ?>
            
            <label for="name">Nama Makanan:</label><br>
            <input type="text" id="name" name="name" value="<?php echo $edit_item ? htmlspecialchars($edit_item['name']) : ''; ?>" required><br>
            
            <label for="description">Deskripsi:</label><br>
            <textarea id="description" name="description" rows="4" required><?php echo $edit_item ? htmlspecialchars($edit_item['description']) : ''; ?></textarea><br>
            
            <label for="price">Harga (Rp):</label><br>
            <input type="number" id="price" name="price" value="<?php echo $edit_item ? htmlspecialchars($edit_item['price']) : ''; ?>" min="0" required><br>
            ...
            <button type="submit" class="btns"><?php echo $edit_item ? 'Simpan Perubahan' : 'Tambah Menu Baru'; ?></button>
            <?php if ($edit_item): ?>
                <a href="homepage-admin-4.php?counter=<?php echo $selected_counter_num; ?>" class="btns-logout">Batal Edit</a> 
            <?php endif; ?>
        </form>
                ...
                <?php if (!empty($menu_items)): ?>
                    <?php foreach ($menu_items as $item): ?>
                        <tr>
                            ...
                            <td>
                                <a href="homepage-admin-4.php?edit_id=<?php echo $item['id']; ?>&counter=<?php echo $selected_counter_num; ?>">Edit</a> | 
                                <a href="homepage-admin-4.php?delete_id=<?php echo $item['id']; ?>" onclick="return confirm('Yakin ingin menghapus menu <?php echo htmlspecialchars($item['name']); ?>?');">Hapus</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
